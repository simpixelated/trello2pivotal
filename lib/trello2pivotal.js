...
class App {
     ...
     */
    run() {
        let cards = this.board.cards.filter(card => !card.closed);
    ...
    writePivotalTrackerRow(card) {
        ...
        let state = STATE_ACCEPTED;//this.cardState(card, type);
        let columnValues = [
            ...
            csvValueSafe(cardEstimate(card))
        ];
    ...
    cardType(card) {
        ...
        for (let i = 0; i < card.idLabels.length; i++) {
            const label = this.labels[card.idLabels[i]];
            if (label) {
                let name = label.name;
    ...
    cardState(card, type) {
        ...
        } else if (contains(listName, "verified")) {
            if (type === TYPE_CHORE) {
        ...
        } else if (contains(listName, "progress")) {
            return STATE_STARTED;
...
function cardEstimate(card) {
    const estimated = /^.*?\([^\d]*(\d+)[^\d]*\).*$/g.exec(card.name);
    const consumed = /^.*?\[[^\d]*(\d+)[^\d]*\].*$/g.exec(card.name);

    if (estimated && estimated.length) {
        return getNearestFibonacci(estimated[1]);
    }

    if (consumed && consumed.length) {
        return getNearestFibonacci(consumed[1]);
    }

    return 0;
}

function getNearestFibonacci(n, x=0, y=1) {
    return y<n?getNearestFibonacci(n,y,x+y):y-n>n-x?x:y;
}
